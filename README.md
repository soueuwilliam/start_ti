...
## 5.3 Batch API: Desconto Automático de 50%

### Princípio

Para tarefas sem necessidade de tempo real, Batch API entrega 50% desconto em todos os modelos.
...
- ✓ Tarefas com SLA >1 hora

### Quando NÃO Usar

- ✗ Chatbots (necessitam resposta imediata)
- ✗ APIs em tempo real
- ✗ Análise interativa
...
# 1. Preparar requisições em JSONL
requests = []
...
print(f"Batch {batch.id} criado. Economizando 50% em tokens!")

# 3. Recuperar resultados após conclusão
# (verificar batch.status periodicamente)
```
...
| Métrica | Valor |
|---------|-------|
| Desconto | 50% em todos os tokens |
| Aplicação | Qualquer tarefa não-tempo-real |
...
## 6.2 Exemplo Integrado: Aplicação Real (RAG + Classificação + Geração)

### Cenário

Sistema de suporte ao cliente que:
1. Classifica intenção do ticket
2. Recupera documentos relevantes (RAG)
...
# Sistema prompt estável (vai para cache)
SYSTEM_PROMPT = """
Você é um especialista em suporte ao cliente.
Seu trabalho:
1. Entender o problema do cliente
2. Recuperar informações relevantes
3. Fornecer solução clara e empática

Categorias de problemas:
- Técnico: erro de software, crash, bug
- Faturamento: cobrança, reembolso, plano
- Conta: acesso, login, autenticação
- Outro: perguntas gerais

[Longa documentação de políticas, procedimentos, exemplos...]
"""  # ~3000 tokens

def process_support_ticket(ticket: dict) -> dict:
    """Processa ticket com roteamento inteligente."""
    
    # PASSO 1: Classificação (modelo barato)
    classification = client.messages.create(
        model="gpt-4.1-nano",  # Nano para classificação
        max_tokens=20,
        messages=[{
            "role": "user",
            "content": f"Classifique: {ticket['mensagem']}\n\nCategorias: técnico, faturamento, conta, outro"
        }]
    ).content[0].text.strip()
    
    category = classification.split(":")[1].strip().lower() if ":" in classification else "outro"
    
    # PASSO 2: RAG - Recuperar contexto relevante
    relevant_docs = retrieve_docs_for_category(category, top_k=3)
    context = "\n".join(relevant_docs)
    
    # PASSO 3: Gerar resposta com cache ativo
    response = client.messages.create(
        model="gpt-4.1-mini",  # Mini para resposta (mais barato que full)
        max_tokens=500,
        system=[
            {
                "type": "text",
    ...
    return {
        ...
        "tokens_cacheados": response.usage.cache_read_input_tokens or 0,
        "custo_estimado": (
            (response.usage.input_tokens - (response.usage.cache_read_input_tokens or 0)) * 0.40 +
            (response.usage.cache_read_input_tokens or 0) * 0.10 +
            response.usage.output_tokens * 1.60
        ) / 1_000_000
    }

# Exemplo de uso
tickets = [
    {"id": "T001", "mensagem": "Meu software está crashando ao abrir"},
    {"id": "T002", "mensagem": "Por que fui cobrado 2x este mês?"},
    {"id": "T003", "mensagem": "Não consigo fazer login na minha conta"},
]

for ticket in tickets:
    result = process_support_ticket(ticket)
    print(f"Ticket {result['ticket_id']}: {result['categoria']}")
...
**Sem otimizações:**
- Usar GPT-4.1 para tudo: $2.00/$8.00
- Sem RAG: 50K+ tokens por ticket
- Custo por ticket: ~$0.10

**Com otimizações:**
- Classificação: nano ($0.10/$0.40)
- Resposta: mini com cache 75% off
- RAG: 3 chunks ~1K tokens (vs 50K sem RAG)
- Custo por ticket: ~$0.01–0.02

**Economia:** ~80% por ticket = $0.08 economizados/ticket

Se processar 10K tickets/mês:
- Sem otimizações: 10K × $0.10 = $1.000/mês
- Com otimizações: 10K × $0.01 = $100/mês
- **Economia: $900/mês = $10.800/ano**

---

## 6.3 Checklist de Otimização Pré-Produção
...
## 🎯 Próximas Ações por Função

### Para Engenheiros

1. **Dia 1:** Audit de prompts atuais — identifique tokens desnecessários
2. **Semana 1:** Implementar cache em system prompts longos
3. **Semana 2:** Adicionar compressão semântica em 2–3 endpoints críticos
4. **Semana 3:** Implementar cascade de modelos para roteamento automático
5. **Mês 1:** Monitorar ganhos de custo e qualidade

### Para Product Managers

1. **Dia 1:** Mapear todos os endpoints por modelo e custo atual
2. **Semana 1:** Priorizar otimizações por ROI (maior economia % primeiro)
3. **Semana 2:** Estabelecer SLA de qualidade e custo por feature
4. **Mês 1:** Comunicar ganhos aos stakeholders (ROI, latência, escalabilidade)

### Para Decision Makers

1. **Imediato:** Revisar matriz de custo-benefício (Parte 6.1)
2. **Esta semana:** Decidir estratégia de modelo (GPT-5 vs GPT-4.1, o-series?)
3. **Este mês:** Aprovar investimento em otimizações identificadas
4. **Trimestral:** Revisar ROI e ajustar estratégia
...
| KPI | Frequência | Alvo | Ação |
|-----|-----------|------|------|
| **Custo/requisição** | Diário | -30% em 3 meses | Ajustar modelo ou prompt |
| **Latência p95** | Diário | <2s | Investigar gargalos |
| **Taxa cache hit** | Diário | >70% | Aumentar system prompt estático |
| **Taxa de erro/retry** | Semanal | <5% | Melhorar qualidade com CoT/few-shot |
| **ROI de otimização** | Mensal | Positivo | Continuar/parar iniciativa |
...
- [ ] Audit completo de custos por endpoint
- [ ] Implementar cache em 80% dos system prompts
- [ ] Compressão semântica em top-5 endpoints

**Ganho esperado:** 25–30% redução de custos

### Mês 2: Inteligência

- [ ] Cascade de modelos com roteamento automático
- [ ] RAG para aplicações com docs longos
- [ ] Few-shot examples em classificadores

**Ganho esperado:** +15–20% redução adicional, +10–15% em qualidade

### Mês 3: Otimização Avançada

- [ ] Integração com Batch API para tarefas não-urgentes
- [ ] Monitoramento contínuo de cache hit rate e reasoning tokens
- [ ] Fine-tuning ou prompt evolution para máxima eficiência

**Ganho esperado:** +10–15% redução final, máxima qualidade/token

**Total esperado após 3 meses:** 50–65% redução de custo + melhoria de qualidade
...
1. **Meça antes de otimizar.** Sem baseline, você não sabe se economizou.
2. **Comece pelos grandes ganhadores.** Cache + RAG + Cascade = ~70% economia.
3. **Sacrifique o mínimo de qualidade.** Usuários notam degradação; salve-se validando com humanos.
4. **Automatize tudo.** Roteamento manual = overhead e inconsistência.
5. **Revise trimestralmente.** Modelos mudam, preços caem, novas técnicas surgem.

### Armadilhas Comuns

| Armadilha | Por Que Piora | Como Evitar |
|-----------|---------------|------------|
| Comprimir demais | Qualidade cai drasticamente | Testar A/B com 10% tráfego |
| Não monitorar cache | Perde ganhos sem saber | Adicionar métrica à dashboard |
| Usar RAG errado | Chunks irrelevantes + confusão | Validar relevância com recall@5 |
| Não revisitar | Modelos melhoram, requer re-opt | Agenda trimestral de revisão |
| Nenhuma validação de qualidade | Economiza $, perde usuários | Métricas de erro obrigatórias |
...
- **Discord Anthropic:** comunidade ativa de usuários
- **OpenAI Community Forum:** Q&A sobre modelos, pricing
- **r/MachineLearning:** discussões acadêmicas
- **LangChain Docs:** frameworks para aplicações de produção
...
APIs de LLM são poderosas mas caras. Custos crescem com volume. Sem otimização, uma aplicação típica gasta **$0.10–1.00 por requisição**.

### Oportunidade

Aplicando **5 técnicas de otimização**, reduzimos custos em **50–65%** mantendo ou melhorando qualidade:

1. **Compressão semântica:** -30–50% input
2. **RAG eficiente:** -70–90% contexto
3. **Controle de output:** -20–40% output
4. **Prompt caching:** -75–90% prefixo reutilizável
5. **Cascade de modelos:** -40–70% uso de modelos caros

### Impacto Financeiro

| Aplicação | Requisições/mês | Custo Sem Otimização | Custo Com Otimização | Economia |
|-----------|-----------------|---------------------|--------------------|----------|
| Chatbot Suporte | 100K | $10.000 | $2.000 | -80% |
| RAG Corporativo | 50K | $7.500 | $1.500 | -80% |
| Classificação | 1M | $10.000 | $1.000 | -90% |
| **Portfólio Médio** | — | **$27.500** | **$4.500** | **-84%** |

**Economia anual (portfólio típico):** ~$276K / ano

### Próximos Passos

- **Semana 1:** Audit de custos atuais
- **Semana 2:** Implementar cache + compressão (ganhos rápidos)
- **Mês 1:** Cascading de modelos + RAG (ganhos máximos)
- **Mês 3:** Monitoramento contínuo e otimização

---

**Versão:** 1.0 | **Data:** Maio 2026 | **Status:** Pronto para Produção